...
public class AdminAuthServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DriverManager.getConnection(JDBC_URL, JDBC_USER, JDBC_PASS)) {
            ...
            if ("login".equals(action)) {
                ...
                handleChangePassword(request, response, conn);
            }

        } catch (SQLException e) {
            e.printStackTrace();
            request.setAttribute("loginError", "Database connection error: " + e.getMessage());
    ...
    private void handleChangePassword(HttpServletRequest request, HttpServletResponse response, Connection conn)
            throws ServletException, IOException, SQLException {
        ...
        if (!newPassword.equals(confirmPassword)) {
            ...
            if (rs.next()) {
                ...
                if (rowsUpdated > 0) {
                    ...
                    request.setAttribute("changeError", "Error updating password. Try again.");
                }
            } else {
